...
import (
	"context"
	"fmt"
	"learngo/custom_error"
	"learngo/internal"
	"learngo/product_web/req"
	"learngo/proto/pb"
	"net/http"
	"strconv"
...
func init() {
	...
	if err != nil {
		zap.S().Errorw("[init] grpc.Dial failed", "msg", err.Error())
		panic(err)
	}
...
func ProductListHandler(c *gin.Context) {
	...
	minPriceStr := c.DefaultQuery("maxPrice", "0")
	minPrice, err := strconv.Atoi(minPriceStr)
	if err != nil {
		zap.S().Error("[ProductListHandler] strconv.Atoi failed", "msg", err.Error())
...
func AddHandler(c *gin.Context) {
	...
	r := ConvertProductReq2pPb(productReq)
...
func ConvertProductReq2pPb(productReq req.ProductReq) *pb.CreateProductItem {
