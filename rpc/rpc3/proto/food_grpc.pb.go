...
type FoodServiceClient interface {
	...
	// 双向流模式
	FullName(ctx context.Context, opts ...grpc.CallOption) (FoodService_FullNameClient, error)
}
...
func (c *foodServiceClient) FullName(ctx context.Context, opts ...grpc.CallOption) (FoodService_FullNameClient, error) {
	stream, err := c.cc.NewStream(ctx, &FoodService_ServiceDesc.Streams[2], "/FoodService/FullName", opts...)
	if err != nil {
		return nil, err
	}
	x := &foodServiceFullNameClient{stream}
	return x, nil
}

type FoodService_FullNameClient interface {
	Send(*FoodStreamRequest) error
	Recv() (*FoodStreamResponse, error)
	grpc.ClientStream
}

type foodServiceFullNameClient struct {
	grpc.ClientStream
}

func (x *foodServiceFullNameClient) Send(m *FoodStreamRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *foodServiceFullNameClient) Recv() (*FoodStreamResponse, error) {
	m := new(FoodStreamResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type FoodServiceServer interface {
	...
	// 双向流模式
	FullName(FoodService_FullNameServer) error
...
func (UnimplementedFoodServiceServer) FullName(FoodService_FullNameServer) error {
	return status.Errorf(codes.Unimplemented, "method FullName not implemented")
}
...
func _FoodService_FullName_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(FoodServiceServer).FullName(&foodServiceFullNameServer{stream})
}

type FoodService_FullNameServer interface {
	Send(*FoodStreamResponse) error
	Recv() (*FoodStreamRequest, error)
	grpc.ServerStream
}

type foodServiceFullNameServer struct {
	grpc.ServerStream
}

func (x *foodServiceFullNameServer) Send(m *FoodStreamResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *foodServiceFullNameServer) Recv() (*FoodStreamRequest, error) {
	m := new(FoodStreamRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var FoodService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "FullName",
			Handler:       _FoodService_FullName_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
